...
import { ArrowRight, Check, Loader2, MapPin, ShieldCheck, X } from 'lucide-react';
...
export function ValorarPropiedadModal({
  open,
  onClose,
  agencies,
}: {
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          onMouseDown={(e) => { if (e.target === e.currentTarget && phase !== 'sending') onClose(); }}
        >
          <motion.div
            ...
            className={`relative max-h-[90vh] w-full overflow-y-auto rounded-2xl bg-white shadow-2xl ${phase === 'select' || phase === 'sending' ? 'max-w-2xl' : 'max-w-lg'}`}
          >
            <button
              type="button"
              onClick={onClose}
              ...
              aria-label="Cerrar"
              className="absolute right-4 top-4 z-10 rounded-full p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 disabled:opacity-40"
            >
              <X size={18} />
            </button>
            ...
            {phase === 'sent' ? (
              <div className="flex flex-col items-center px-8 py-14 text-center">
                ...
                <h2 className="text-2xl font-bold tracking-tight text-slate-900">¡Solicitud enviada!</h2>
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="mt-8 inline-flex items-center justify-center rounded-md bg-[#FF8000] px-8 py-4 text-sm font-bold uppercase tracking-widest text-white transition-all hover:bg-[#E67300] active:scale-[0.98]"
                >
                  Entendido
                </button>
              </div>
                ...
                {matched.length === 0 ? (
                  <div className="mt-6 rounded-xl border border-slate-100 bg-slate-50 p-6 text-center">
                    ...
                    <button
                      type="button"
                      onClick={onClose}
                      className="mt-4 inline-flex items-center gap-2 rounded-full bg-[#FF8000] px-6 py-3 text-sm font-bold text-white transition-colors hover:bg-[#E67300] active:scale-95"
                    >
                  ...
                  <>
                    <div className="mt-4 flex items-center justify-between">
                      ...
                      <button
                        type="button"
                        onClick={() => setSelected(selected.length === matched.length ? [] : matched.map((a) => a.id))}
                        ...
                        return (
                          <button
                            ...
                            className={`group relative flex items-center gap-3 rounded-xl border p-4 text-left transition-all duration-200 ${
                              isSelected
                                ? 'border-[#FF8000] bg-orange-50/60 ring-2 ring-[#FF8000]/20'
                                : 'border-slate-200 bg-white hover:border-[#FF8000]/40'
                            }`}
                          >
                            <div
                              className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-lg text-sm font-bold text-white shadow-sm"
                              style={{ backgroundColor: a.color_hex ?? '#FF8000' }}
                            >
                              {a.logo_url ? (
                                <img src={a.logo_url} alt={a.nombre_comercial} className="h-full w-full object-cover" />
                              ) : (
                                a.nombre_comercial.split(' ').map((w) => w[0]).slice(0, 2).join('').toUpperCase()
                              )}
                            </div>
                            <div className="min-w-0 flex-1">
                              <p className="truncate text-sm font-bold text-slate-900">{a.nombre_comercial}</p>
                            ...
                            <span
                              className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 transition-colors ${
                                isSelected ? 'border-[#FF8000] bg-[#FF8000]' : 'border-slate-300 bg-white'
                              }`}
                            >
